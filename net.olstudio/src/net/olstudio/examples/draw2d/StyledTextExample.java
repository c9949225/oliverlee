...
public class StyledTextExample {
	...
	private void create(final Shell shell) {
		...
		ranges[0].background = shell.getDisplay().getSystemColor(
				SWT.COLOR_YELLOW);
		ranges[0].fontStyle = SWT.BOLD;

		ranges[1] = new StyleRange(5, 2, blue, null);
		ranges[1].background = shell.getDisplay().getSystemColor(
				SWT.COLOR_YELLOW);

		ranges[2] = new StyleRange(8, 2, red, null);
		ranges[2].background = shell.getDisplay().getSystemColor(
				SWT.COLOR_YELLOW);

		ranges[3] = new StyleRange(11, 6, green, null);
		ranges[3].underline = true;
		ranges[3].background = shell.getDisplay().getSystemColor(
				SWT.COLOR_YELLOW);
		ranges[3].fontStyle = SWT.BOLD;

		ranges[4] = new StyleRange(18, 8, orange, null);
		ranges[4].underline = true;
		ranges[4].background = shell.getDisplay().getSystemColor(
				SWT.COLOR_YELLOW);
		ranges[4].fontStyle = SWT.BOLD;

		ranges[5] = new StyleRange(27, 11, blue, null);
		ranges[5].background = shell.getDisplay().getSystemColor(
				SWT.COLOR_YELLOW);

		ranges[6] = new StyleRange(39, 9, red, null);
		ranges[6].background = shell.getDisplay().getSystemColor(
				SWT.COLOR_YELLOW);
