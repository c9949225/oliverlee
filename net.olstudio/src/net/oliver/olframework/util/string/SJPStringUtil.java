 ...
 * 		
 */
public final class SJPStringUtil {
	...
	public static String IdCardTransfer(String numToTransfer) throws Exception {
		...
		if (numToTransfer == null || numToTransfer.length() != 15)
			return numToTransfer;

		String newId = "";

		String tmp17 = numToTransfer.substring(0, 6) + "19"
				+ numToTransfer.substring(6);
		numToTransfer = tmp17;
		String code; // 第18位校验位
		int num = 0;
		for (int i = 18; i > 1; i--)
			num += (int) Math.pow(2, i - 1)
					% 11
					* Integer.valueOf("" + numToTransfer.charAt(18 - i))
							.intValue();
		num %= 11;
		switch (num) {
		case 0:
		...
		newId = numToTransfer + code;
		return newId;
	}

	/**
	 * 判断一个字符是否是汉字
	 * 如果有了汉字，其实是字符串短了，所以在截的地方要往前移动
	 */
	 ...
	 * 给定一个字符串，判断里面包含多少个汉字
	 */
	public static int getCharacterNum(String input) {
		int result = 0;
		byte[] str_byte = null;
		for (int i = 0; i < input.length(); i++) {
			str_byte = input.substring(i, i + 1).getBytes();
			if (str_byte.length == 2) {
				result += 1;
			}
		}
	...
	/**
	 * 给定一个字符串，判断该字符串的非汉字(字节)长度
	 */
	public static int getByteNumOfString(String input) {
	 ...
	 * @return
	 */
	public static List getCharacterFromByte(byte[] buf, int len) {
		...
		for (int i = 0; i < size; i++) {
			...
			System.arraycopy(buf, i * len, temp, 0, len);

			byte mybyte = temp[0];
			if (mybyte == 32) {
				continue;
			}
			resultlist.add(new String(temp, 0, len));
		}
		return resultlist;
	}
	...
	public static String getFileNameFromPath(String path) {
		...
		//            return "";
		String[] ps = SJPStringUtil.split(path, File.separator);
